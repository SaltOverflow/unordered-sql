    ...
    def format_statement(self, text: str, statement: ast.Statement) -> str:
        ...
        for idx, select in enumerate(statement.selects):
            ...
            for field in select.fields:
                ...
                if field.alias_name is not None:
                    field_string += " AS "
                    start, end = (
                        field.alias_name.start,
                        field.alias_name.start + field.alias_name.length,
                    )
                    ...
                    if (
                        ...
                        and token.has_end_sequence == False
                    ):
                        field_string += '"'
        ...
        for idx, from_ in enumerate(statement.froms):
            ...
            for field in from_.fields:
                ...
                if isinstance(field.table_name, ast.ValueExpression) and isinstance(
                    field.table_name.token, tok.NameToken
                ):
        ...
        for idx, where in enumerate(statement.wheres):
            if idx == 0:
                ...
                ret += "AND "
            if (
                isinstance(where.expression, ast.BinaryExpression)
                and tok.operator_to_precedence[where.expression.operator.type]
                > tok.operator_to_precedence[tok.NameTokenType.AND]
            ):
                ret += "("
    ...
    def format_expression(self, text: str, expression: ast.Expression) -> str:
        ...
        if isinstance(expression, ast.ValueExpression):
            start, end = (
                ...
                expression.token.start + expression.token.length,
            )
            ...
            op_start, op_end = (
                ...
                expression.operator.start + expression.operator.length,
            )
